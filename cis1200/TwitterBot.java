...
public class TwitterBot {
    ...
    public TwitterBot(BufferedReader br, int tweetColumn, NumberGenerator ng) {
        ...
        List<List<String>> trainingData = TweetParser.csvDataToTrainingData(br, tweetColumn);
    ...
    public void writeStringsToFile(List<String> stringsToWrite, String filePath, boolean append) {
        ...
        // Complete this method.
        // NOT SURE HOW TO INSTANTIATE THE BufferedWriter object it is not very clear
        ...
        try {
            ...
            for (String tweet : stringsToWrite) {
                bw.write(tweet);
                bw.newLine();
            }
        } catch (IOException e) {
    ...
    public String generateTweet(int numWords) {
        ...
        if (numWords < 0) {
            throw new IllegalArgumentException("numwords cannot be less than 0");
        }

        if (!mc.hasNext() || numWords == 0 || !mc.hasNext() || mc.chain.size() == 0) {
            return "";
        }

        String tweet = mc.next();
        int i = 0;
        while (i < numWords - 1) {
            if (mc.hasNext()) {
                String currWord = " " + mc.next();
                tweet += currWord;
                i++;
            } else {
