...
public class MarkovChain implements Iterator<String> {
    ...
    void addBigram(String first, String second) {
        ...
        // check if the pd exists or not
        if (pd == null) {
            ProbabilityDistribution<String> pd2 = new ProbabilityDistribution<>();
            pd2.record(second);
            chain.put(first, pd2);
        } else {
            pd.record(second);
    ...
    public void train(Iterator<String> sentence) {
        if (sentence == null) {
            throw new IllegalArgumentException("The sentence Iterator is null");
        }
        ...
        String currWord = sentence.next();
        startWords.record(currWord);

        while (sentence.hasNext()) {
            String nextWord = sentence.next();
            addBigram(currWord, nextWord);
            currWord = nextWord;
        }

        addBigram(currWord, END_TOKEN);
    }
    ...
    public boolean hasNext() {
        return !currWord.equals(END_TOKEN);
    }
    ...
    public String next() {
        if (!hasNext()) {
            throw new NoSuchElementException("there are no more words on the walk");
        }

        String nextWord = currWord;
        if (!nextWord.equals(END_TOKEN)) {
            ProbabilityDistribution<String> pd = chain.get(currWord);
            if (pd != null) {
                currWord = pd.pick(ng);
            } else {
                currWord = END_TOKEN;
