...
public class TweetParser {
    ...
    static String extractColumn(String csvLine, int csvColumn) {
        // NEED TO ADD TO CHECK IF THE APPROPRIATE COLUMN
        if (csvLine == null) {
            return null;
        }
        String[] s = csvLine.split(",");
        if (csvColumn < 0 || csvColumn > s.length - 1) {
            return null;
        }
        return s[csvColumn];
    }
    ...
    static List<String> csvDataToTweets(BufferedReader br, int tweetColumn) {
        // CHECK WHAT DOES IT MEAN FOR THE TWEET COLUMN TO BE OUT OF BOUNDS
        FileLineIterator fli = new FileLineIterator(br);
        List<String> tweets = new ArrayList<>();

        while (fli.hasNext()) {
            String currTweet = extractColumn(fli.next(), tweetColumn);
            if (currTweet != null) {
    ...
    static List<List<String>> parseAndCleanTweet(String tweet) {
        List<List<String>> cleanParseTweet = new ArrayList<>();
        String cleanedTweet = removeURLs(tweet);
        List<String> splitTweets = tweetSplit(cleanedTweet);
        for (String s : splitTweets) {
            List<String> cleanSentence = parseAndCleanSentence(s);
            if (!cleanSentence.isEmpty()) {
                cleanParseTweet.add(cleanSentence);
            }
        }
        return cleanParseTweet;
    }
    ...
    public static List<List<String>> csvDataToTrainingData(BufferedReader br, int tweetColumn) {
        ...
        List<String> tweetsExtracted = csvDataToTweets(br, tweetColumn);
        for (String tweet : tweetsExtracted) {
            List<List<String>> cleaned = parseAndCleanTweet(tweet);
            if (!cleaned.isEmpty()) {
                trainingData.addAll(cleaned);
            }
        }
        return trainingData;
    }
